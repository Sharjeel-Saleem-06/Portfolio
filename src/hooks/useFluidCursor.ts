...
const useFluidCursor = () => {
  ...
  const violetBases = [
    ...
    { r: 108 / 255, g: 59 / 255, b: 255 / 255 }, // #6C3BFF
  ];
  ...
  function refreshAccentPalette() {
    try {
      ...
      let accent = cs.getPropertyValue("--accent").trim();
      
      // If not found, try getting it from the theme-specific selector
      if (!accent) {
        const isDark = root.getAttribute("data-theme") === "dark";
        accent = isDark ? "#3B82F6" : "#3B82F6"; // Default blue accent
      }
      ...
      if (accent) {
        ...
        if (accent.startsWith("#")) {
          ...
          // Try HSL format
          accentPalette = [hslTripletToRGB(accent)].map((rgb) => ({
            r: rgb.r / 255,
            g: rgb.g / 255,
            b: rgb.b / 255,
          }));
  ...
  const displayShaderSource = `
       ...
       uniform vec2 texelSize;
  uniform float darkMode; // 1.0 when dark theme active
      ...
      // In dark mode, darken the core (areas of higher density/alpha) toward black for a subtler look
      if (darkMode > 0.5) {
        float core = clamp(a, 0.0, 1.0);
        // core closer to 1.0 becomes darker; retain hue on edges (lower alpha)
        c = mix(c, vec3(0.0), core * 0.85);
      }
  ...
  function drawDisplay(target) {
    ...
    // Pass dark mode flag (1.0 for dark)
  ...
  function generateColor() {
    ...
    if (accentPalette.length) {
      // 30% chance to pick a violet base to introduce subtle purple hue
      const useViolet = Math.random() < 0.3;
      const base = useViolet
        ? violetBases[Math.floor(Math.random() * violetBases.length)]
        : accentPalette[Math.floor(Math.random() * accentPalette.length)];
      // Jitter within cool spectrum (shift towards violet or cyan)
      const jitterScale = useViolet ? 0.08 : 0.12; // slightly tighter jitter for violets
      const jitter = () => (Math.random() - 0.5) * jitterScale; // small variation
      let r = clamp01(base.r + jitter());
    ...
    // Fallback: deterministic cool hue range
    const hue = 210 + Math.random() * 65; // 210-275
  ...
  // Converts a CSS variable triplet string like "183 65% 35%" (h s% l%) to RGB {r,g,b}
  function hslTripletToRGB(triplet) {
    // Expect formats: "183 65% 35%" or with extra spaces
